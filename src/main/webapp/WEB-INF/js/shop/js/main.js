$(document).ready(function () {
    ...
    $('.add-cart-large').each(function (i, el) {
        $(el).click(function () {
            ...
            var price=$(this).parent().find('h4').get(1).innerHTML;
    ...
    $('.add_to_cart').click(function () {
        ...
        var p=price.slice(1);
        var id=$(productCard).find('.product_id').get(0).innerHTML;
        var uri='/cart/addCart';
        $.post(uri,{pid:id,money:p,num:1},function(result){
            if (result==1){
                console.log("添加成功");
            }
        });
        ...
        setTimeout(function () {
            $('div.floating-cart').remove();
            $('body').removeClass('MakeFloatingCart');
            var cartItem = '<div class=\'cart-item\'><div class=\'img-wrap\'><img src=\'' + productImage + '\' alt=\'\' /></div><span>' + productName + '</span><strong class=\'price\'>'+price+'</strong><input type=\'hidden\' class=\'pid\' value=\''+id+'\'><div class=\'cart-item-border\'></div><div class=\'delete-item\'></div></div>';
            $('#cart .empty').hide();
            $('#cart').append(cartItem);
            $('#checkout').fadeIn(500);
            $('#cart .cart-item').last().addClass('flash').find('.delete-item').click(function () {
                $(this).parent().fadeOut(300, function () {
                    ...
                    var p=$(this).find('.price').html().slice(1);
                    var uri='/cart/delCart';
                    $.post(uri,{pid:id,money:p,num:1},function(result){
                        if (result==1){
                            console.log("删除成功");
                        }
