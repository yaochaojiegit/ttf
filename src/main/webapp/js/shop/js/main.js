$(document).ready(function () {
    ...
    $('.add_to_cart').click(function () {
        ...
        var pid=$(productCard).find('.product_id').get(0).innerHTML;
        $('body').append('<div class="floating-cart"></div>');
        var uri="/userCheck";
        $.post(uri,{},function (result) {
            if (result == null||result=="") {
                layer.alert("尚未登录,前去登录",function () {
                    location.href="UserLogin.html";
                });
               
            }else {
                var uri="/cart/addCart";
                $.post(uri,{pid:pid,num:1,money:price.substring(1)},function (result) {
                    if (result==1){
                        console.log("添加成功");
                    } else {
                        console.log("添加失败");
        ...
        setTimeout(function () {
            $('div.floating-cart').remove();
            $('body').removeClass('MakeFloatingCart');
            var cartItem = '<div class=\'cart-item\'><div class=\'img-wrap\'><img src=\'' + productImage + '\' alt=\'\' /></div><span>' + productName + '</span><strong>'+price+'</strong><span></span><div class=\'cart-item-border\'></div><div class=\'delete-item\'></div></div>';
            $('#cart .empty').hide();
            $('#cart').append(cartItem);
            $('#checkout').fadeIn(500);
            $('#cart .cart-item').last().addClass('flash').find('.delete-item').click(function () {
